						...
						unsigned long ret_ip)
{
	...
	if (is_kernel_core_data(addr_val) || is_kernel_rodata(addr_val) || __is_kernel_text(addr_val)) {
		...
		hash = hash_64(hash_a, MYINST_RW_RET_IP_HASHTABLE_BITS);
		if (atomic_cmpxchg(&rw_ret_ip_counter[hash], 0, 1)) {
			return true;
		}
		atomic_inc(&rw_ret_ip_counter_usage);

		// report
		kasan_disable_current();
		trace_rw(ret_ip, addr_val, size, write);
