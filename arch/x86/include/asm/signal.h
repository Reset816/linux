...
{
	bool ret;
	unsigned int __bit = (unsigned int)(_sig - 1);
	unsigned int __word =
		__bit / (8U * (unsigned int)sizeof(unsigned long));
	unsigned int __shift =
		__bit % (8U * (unsigned int)sizeof(unsigned long));
	unsigned long __val = ((unsigned long *)set)[__word];
	ret = (bool)((__val >> __shift) & 1UL);
	return ret;
}
