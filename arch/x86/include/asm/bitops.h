...
static __always_inline void arch_set_bit(long nr, volatile unsigned long *addr)
{
	if (__builtin_constant_p(nr)) {
		...
		byte_addr += (nr >> 3);
		mask = (unsigned char)(1U << (nr & 7));
		value = (unsigned char)*byte_addr;
		value |= mask;
		*byte_addr = value;
	} else {
		unsigned long index;
		...
		unsigned long mask;
		volatile unsigned long *word_addr;
		unsigned long value;

		index = (unsigned long)nr / (sizeof(unsigned long) * 8);
		bit_pos = (unsigned long)nr & ((sizeof(unsigned long) * 8) - 1);
		mask = 1UL << bit_pos;
		word_addr = addr + index;
		value = *word_addr;
		value |= mask;
		*word_addr = value;
	}
					   ...
					   volatile unsigned long *addr)
{
	...
	index = nr / (sizeof(unsigned long) * 8);
	bit_pos = nr & ((sizeof(unsigned long) * 8) - 1);
	mask = 1UL << bit_pos;
	word_addr = addr + index;
	value = *word_addr;
	value |= mask;
...
static __always_inline void arch___clear_bit(unsigned long nr,
					     volatile unsigned long *addr)
{
	const unsigned long word_bits = sizeof(unsigned long) * 8;
	volatile unsigned long *ptr = addr + (nr / word_bits);
...
arch___test_and_set_bit(unsigned long nr, volatile unsigned long *addr)
{
	...
	offset = nr / (sizeof(unsigned long) * 8UL);
	mask = 1UL << (nr % (sizeof(unsigned long) * 8UL));
	value = addr[offset];
	oldbit = (value & mask) != 0;
	value |= mask;
	addr[offset] = value;
...
constant_test_bit_acquire(long nr, const volatile unsigned long *addr)
{
	...
	unsigned char value = bytes[nr >> 3];
	oldbit = (value & (1 << (nr & 7))) != 0;
