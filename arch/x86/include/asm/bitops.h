					     ...
					     volatile unsigned long *addr)
{
	...
	volatile unsigned long *p = addr + index;
	unsigned long val = *p;
...
{
	bool negative;
	volatile unsigned char *byte_addr = (volatile unsigned char *)addr;
	unsigned char mask_byte = (unsigned char)mask;
	unsigned char old_byte = *byte_addr;
	unsigned char new_byte = old_byte ^ mask_byte;

	*byte_addr = new_byte;
	negative = ((signed char)new_byte) < 0;
	return negative;
}
					    ...
					    volatile unsigned long *addr)
{
	if (__builtin_constant_p(nr)) {
		...
		unsigned long index = bitnr / (sizeof(unsigned long) * 8UL);
		unsigned long mask = 1UL
				     << (bitnr % (sizeof(unsigned long) * 8UL));
		__atomic_fetch_xor((unsigned long *)&addr[index], mask,
				   __ATOMIC_SEQ_CST);
	} else {
		unsigned long bitnr = (unsigned long)nr;
		unsigned long index = bitnr / (sizeof(unsigned long) * 8UL);
...
{
	bool oldbit;
	unsigned long word_index = nr / (sizeof(unsigned long) * 8);
	unsigned long bit_index = nr % (sizeof(unsigned long) * 8);
	volatile unsigned long *word_ptr = addr + word_index;
	unsigned long word_val = *word_ptr;
...
constant_test_bit_acquire(long nr, const volatile unsigned long *addr)
{
	...
	const volatile unsigned char *addr_bytes =
		(const volatile unsigned char *)addr;

	oldbit = (addr_bytes[nr >> 3] & (1 << (nr & 7))) != 0;
...
static __always_inline unsigned long __fls(unsigned long word)
{
	...
	word = BITS_PER_LONG - 1 - __builtin_clzl(word);
	return word;
}
...
static __always_inline int fls(unsigned int x)
{
	 ...
	 */
	if (x) {
		int shift = (int)(sizeof(unsigned int) * 8) - 1;
		r = shift - __builtin_clz(x);
	} else {
		r = -1;
	}
#elif defined(CONFIG_X86_CMOV)
